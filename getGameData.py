...
def run(debug=False):
    ...
    import sys
    #if len(sys.argv) == 1:  sys.argv += ["--command", "python"]
    FLAGS(sys.argv)
    
    config = gameConfig.Config(
        vers=None, # vers is None... unless a specific game version is desired
        mapName="Sequencer",  # smaller maps load faster?
        agentRaces=[c.RANDOM], bots=[(c.RANDOM, c.EASY)], # simple 1v1 in dummy game 
    )
